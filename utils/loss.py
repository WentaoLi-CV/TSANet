...
class fusion_loss(nn.Module):
    ...
    def forward(self, image_visible, image_infrared, image_fused, max_ratio=4, ssim_vis_ratio=1, ssim_ir_ratio=1,
        ...
        loss_ssim_vis = ssim_vis_ratio * self.loss_func_ssim(image_visible, image_fused)
        loss_ssim_ir = ssim_ir_ratio * self.loss_func_ssim(image_infrared_gray, image_fused_gray)
        loss_ssim = ssim_ratio * (loss_ssim_vis + loss_ssim_ir)
...
class fusion_prompt_loss(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused):
        ...
        loss, ssim_loss, max_loss, color_loss, grad_loss = self.fusion_loss(image_A, image_B, image_fused,
                                                                            max_ratio=0.1, ssim_ratio=1,
                                                                            text_ratio=10, color_ratio=12)
        return loss, ssim_loss, max_loss, color_loss, grad_loss